...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout,dataset_type):
        ...
        if dataset_type == "EN":
            ...
            self.bert_prompt = RobertaModel.from_pretrained('roberta-base').requires_grad_(True)
            ...
            self.bert_prompt = BertModel.from_pretrained(bert).requires_grad_(True)
    ...
    def forward(self, **kwargs):
        ...
        image_feature = torch.cat((
                image_feature[:, :1, :]+self.soft_prompt_dropout(self.soft_prompt_embeds.expand(batch_size, -1, -1)),
                image_feature[:, 1:, :]
            ), dim=1)
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_prompt.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_prompt.pkl')))
        results0 = self.test(self.test_loader)
        final = recorder.printTest(results0)
        #print(results0)
        return results0, os.path.join(self.save_param_dir, 'parameter_prompt.pkl')
