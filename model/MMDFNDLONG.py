...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        #self.bert = RobertaModel.from_pretrained('roberta-large').requires_grad_(False)
...
##continue
        ...
        text_label_pred_list = torch.cat((text_label_pred_list[0], text_label_pred_list[1], text_label_pred_list[2], text_label_pred_list[3],
                                     text_label_pred_list[4], text_label_pred_list[5], text_label_pred_list[6]))
        #image
        image_only_output = []
        image_label_pred = []
        final_image_feature = []
        ...
        image_label_pred_list = torch.cat((image_label_pred_list[0], image_label_pred_list[1], image_label_pred_list[2], image_label_pred_list[3],
                                     image_label_pred_list[4], image_label_pred_list[5], image_label_pred_list[6]))
        # fusion
        fusion_only_output = []
        fusion_label_pred = []
        ...
        fusion_label_pred_avg = fusion_label_pred_avg / 7
        ...
        final_label_pred_list = torch.cat((final_label_pred_list[0], final_label_pred_list[1], final_label_pred_list[2], final_label_pred_list[3],
                                     final_label_pred_list[4], final_label_pred_list[5], final_label_pred_list[6]))
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_mmdfndlong.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_mmdfndlong.pkl')))
        results0,results1,results2,results3 = self.test(self.test_loader)
        print(results0)
        return results0, os.path.join(self.save_param_dir, 'parameter_cliplong.pkl')
