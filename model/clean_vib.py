...
import models_mae
from utils.utils import data2gpu, Averager, metrics, Recorder, clipdata2gpu,promptdata2gpu
...
class VIBLayer(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.encoder_mean = nn.Linear(in_features, out_features)
        self.encoder_std = nn.Linear(in_features, out_features)
    ...
    def forward(self, x):
        ...
        return z, uncertainty, mean, log_std
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout,dataset_type):
        ...
        if dataset_type == "EN":
            ...
            self.bert_prompt = RobertaModel.from_pretrained('roberta-base').requires_grad_(False)
            self.ClipModel,preprocess = longclip.load("/root/autodl-tmp/domain_model/model/longclip-B.pt", device="cuda")
        else:
            self.bert = BertModel.from_pretrained(bert).requires_grad_(False)
            self.bert_prompt = BertModel.from_pretrained(bert).requires_grad_(False)
        ...
        self.emb_size = 320
        self.text_domain_prompt_mlp = nn.Linear(self.emb_size,1)
        self.image_domain_prompt_mlp = nn.Linear(self.emb_size,1)
        self.mm_domain_prompt_mlp = nn.Linear(self.emb_size,1)
        self.soft_prompt_embeds = nn.Parameter(torch.zeros(1, 1, 768))
        self.soft_prompt_dropout = nn.Dropout(0.2)
        self.text_vib = VIBLayer(320, 320)
        self.image_vib = VIBLayer(320, 320)
        self.fusion_vib = VIBLayer(320, 320)
    ...
    def forward(self, **kwargs):
        ...
        input_data = {'input_ids':kwargs['input_ids'],'attention_mask':kwargs['attention_mask'],'text_loss_ids':kwargs['text_loss_ids']}
        
        outputs = self.bert_prompt(**{k: v for k, v in input_data.items() if k in inspect.signature(self.bert_prompt.forward).parameters})
        outputs_at_mask = {k: self.extract_at_mask(v, input_data) for k, v in outputs.items()}
        text_feature_prompt = outputs_at_mask['last_hidden_state'].unsqueeze(1).repeat(1, 197, 1) 
        text_feature = self.bert(inputs, attention_mask=masks)[0]+text_feature_prompt
        
        image = kwargs['image']
        #image_prompt = kwargs['image_prompt']
        image_feature = self.image_model.forward_ying(image) # ([64, 197, 768])
        image_feature = torch.cat((
                image_feature[:, :5, :]+self.soft_prompt_dropout(self.soft_prompt_embeds.expand(batch_size, -1, -1)),
                image_feature[:, 5:, :]
            ), dim=1)
        #image_feature = self.bert(inputs, attention_mask=masks)[0]
        clip_image = kwargs['clip_image']#torch.Size([64, 3, 224, 224])
        clip_text = kwargs['clip_text']
        #clip_image_prompt = kwargs['clip_image_prompt']
        with torch.no_grad():
        ...
        #clip_image_feature = clip_image_feature+0.1*self.soft_prompt_dropout2(self.image2text(self.text2image(self.clip_text_soft_prompt_embeds.expand(batch_size, -1, -1)))) # ([64, 512])
        #
        # clip_text_feature = clip_text_feature+0.1*self.clip_text_soft_prompt_embeds.expand(batch_size, -1, -1)  # ([64, 512])
        ...
        text_atn_feature = self.text_attention(self.bert(inputs, attention_mask=masks)[0],masks)
        #text_atn_feature = outputs_at_mask['last_hidden_state']+self.text_attention(self.bert(inputs, attention_mask=masks)[0],masks)
        ...
        text = text_gate_spacial_expert_value[0]
        image = image_gate_spacial_expert_value[0]
        ...
        #print(fusion_share_feature.size())
...
##continue
        ...
        text_weight = torch.sigmoid(self.text_domain_prompt_mlp(text_gate_expert_value[0]+text_gate_expert_value[1]))
        text_two_task.append(self.text_classifier(text_gate_expert_value[0]*(1-text_weight)+text_gate_expert_value[1]*text_weight).squeeze(1))
        text_two_task.append(self.text_classifier_Mu(text_gate_expert_value[1]).squeeze(1))

        image_weight = torch.sigmoid(self.image_domain_prompt_mlp(image_gate_expert_value[0]+image_gate_expert_value[1]))
        image_two_task.append(self.image_classifier(image_gate_expert_value[0]*(1-image_weight)+image_gate_expert_value[1]*image_weight).squeeze(1))
        image_two_task.append(self.image_classifier_Mu(image_gate_expert_value[1]).squeeze(1))

        mm_weight = torch.sigmoid(self.mm_domain_prompt_mlp(fusion_gate_expert_value0[0]+fusion_gate_expert_value0[1]))
        fusion_two_task.append(self.fusion_classifier(fusion_gate_expert_value0[0]*(1-mm_weight)+fusion_gate_expert_value0[1]*mm_weight).squeeze(1))
        ...
        text_z, text_uncertainty, _, _ = self.text_vib(text_gate_expert_value[0])
        image_z, image_uncertainty, _, _ = self.image_vib(image_gate_expert_value[0])
        fusion_z, fusion_uncertainty, _, _ = self.fusion_vib(fusion_gate_expert_value0[0])

        fake_text_z, fake_text_uncertainty, _, _ = self.text_vib(text_gate_expert_value[1])
        fake_image_z, fake_image_uncertainty, _, _ = self.image_vib(image_gate_expert_value[1])
        fake_fusion_z, fake_fusion_uncertainty, _, _ = self.fusion_vib(fusion_gate_expert_value0[1])

        text_conf = 1.0 / (text_uncertainty + 1e-10)
        image_conf = 1.0 / (image_uncertainty + 1e-10)
        fusion_conf = 1.0 / (fusion_uncertainty + 1e-10)

        fake_text_conf = 1.0 / (fake_text_uncertainty + 1e-10)
        fake_image_conf = 1.0 / (fake_image_uncertainty + 1e-10)
        fake_fusion_conf = 1.0 / (fake_fusion_uncertainty + 1e-10)

        total_conf = text_conf + image_conf + fusion_conf
        fake_total_conf = fake_text_conf + fake_image_conf + fake_fusion_conf

        text_weight = text_conf / total_conf
        image_weight = image_conf / total_conf
        fusion_weight = fusion_conf / total_conf

        fake_text_weight = fake_text_conf / fake_total_conf
        fake_image_weight = fake_image_conf / fake_total_conf
        fake_fusion_weight = fake_fusion_conf / fake_total_conf

        multi_label_feature = (text_z * text_weight + 
                               image_z * image_weight + 
                               fusion_z * fusion_weight)

        fake_news_feature = (fake_text_z * fake_text_weight + 
                             fake_image_z * fake_image_weight + 
                             fake_fusion_z * fake_fusion_weight)
...
class Trainer():
    ...
    def train(self):
        self.model = MultiDomainPLEFENDModel(self.emb_dim, self.mlp_dims, self.bert, 320, self.dropout,self.dataset_type)
        if self.use_cuda:
        ...
        for epoch in range(self.epoches):
            ...
            for step_n, batch in enumerate(train_data_iter):
                batch_data = promptdata2gpu(batch)
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'clean.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'clean.pkl')))
        results0 = self.test(self.test_loader)
        final = recorder.printTest(results0)
        #print(results0)
        return results0, os.path.join(self.save_param_dir, 'clean.pkl')
