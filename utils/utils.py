...
def promptdata2gpu(batch):
    batch_data = {
        ...
        'input_ids': batch[7].cuda(),
        'attention_mask': batch[8].cuda(),
        'text_loss_ids': batch[9].cuda(),
        'image_prompt': batch[10].cuda(),
        'clip_image_prompt': batch[11].cuda(),
    }
    return batch_data
"""
def promptdata2gpu(batch):
...
def metricsTrueFalse(y_true, y_pred, category, category_dict):
    y_pred = [item for sublist in y_pred for item in (sublist if isinstance(sublist, list) else [sublist])]
    y_true = [item for sublist in y_true for item in (sublist if isinstance(sublist, list) else [sublist])]
    y_GT = y_true
...
def metrics(y_true, y_pred, category, category_dict):
    ...
    y_pred = np.around(np.array(y_pred)).astype(int)
    metrics_by_category['metric'] = f1_score(y_true, y_pred, average='macro')
    #metrics_by_category['metric'] = f1_score(y_true, y_pred, average='macro', labels=[0, 1])
    metrics_by_category['recall'] = recall_score(y_true, y_pred, average='macro')
    metrics_by_category['precision'] = precision_score(y_true, y_pred, average='macro')
    metrics_by_category['acc'] = accuracy_score(y_true, y_pred)
    ...
    for c, res in res_by_category.items():
        ...
        metrics_by_category[c] = {
            'precision': precision_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int),
                                         average='macro',zero_division=1).round(4).tolist(),
            'recall': recall_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int),
                                   average='macro',zero_division=1).round(4).tolist(),
            'fscore': f1_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro',zero_division=1).round(
                4).tolist(),
        ...
        metrics_by_category[c] = {
            'precision': round(
                precision_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro'), 4),
            'recall': round(
                recall_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro'), 4),
...
class Recorder():
    ...
    def add(self, x):
        ...
        safe_print(f"curent {json.dumps(self.cur, ensure_ascii=False)}")
        return self.judge()
    ...
    def showfinal(self):
        #print("Max", self.max)
        safe_print(f"Max {json.dumps(self.max, ensure_ascii=False)}")

    def printTest(self,text):
        safe_print(f"Final {json.dumps(self.max, ensure_ascii=False)}")
